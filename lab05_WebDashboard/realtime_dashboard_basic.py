...
# Deques with max length 20 for rolling window
...
def on_connect(client, userdata, flags, rc):
    ...
    # Replace with your student ID topic
    client.subscribe("esp32/12345678/data")

def on_message(client, userdata, msg):
    global temperature_data, humidity_data, light_data, distance_data, time_data
    try:
        payload = json.loads(msg.payload.decode('utf-8'))
        temperature_data.append(payload.get("temperature", 0))
        humidity_data.append(payload.get("humidity", 0))
        light_data.append(payload.get("light", 0))
        distance_data.append(payload.get("distance", 0))
...
# Standard vertical stacked layout (Baseline)
app.layout = html.Div([
